...
public class OrderController {
    ...
    public String showOrder(HttpSession session, String[] orderItemId,Model model) {
        ...
        //订单信息
        //获取当前用户的购物车信息
        List<Shopcart> cartItems = new ArrayList<>();
		float total = 0;

		for (String strId : orderItemId) {
			int id = Integer.parseInt(strId);
			Shopcart sc = shopCartService.selectCartByKey(new ShopcartKey(user.getUserid(),id));
			total += goodsService.selectById(id).getPrice() * sc.getGoodsnum();
			cartItems.add(sc);
		}
        ...
        session.setAttribute("oldTotalPrice", oldTotalPrice);
        session.setAttribute("goodsAndImage", goodsAndImage);
    ...
    public Msg orderFinish(Float oldPrice, Float newPrice, Boolean isPay, Integer addressid,HttpSession session) {
        ...
        //获取订单信息
        List<Goods> shopCart = (List<Goods>) session.getAttribute("goodsAndImage");
        ...
        //把订单项写入orderitem表中
        for (Goods cart : shopCart) {
            orderService.insertOrderItem(new OrderItem(null, orderId, cart.getGoodsid(), cart.getNum()));
        }
        
        
        //删除购物车
        for (Goods cart : shopCart) {
            shopCartService.deleteByKey(new ShopcartKey(user.getUserid(),cart.getGoodsid()));
        }

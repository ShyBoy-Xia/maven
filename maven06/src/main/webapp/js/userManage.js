...
$(document).on("click",".templatemo-delete-btn",function () {
    ...
    swal({
        ...
        function () {
            ...
            $.ajax({
                url: "/maven06/admin/user/delete/" + goodsid,
                type: "DELETE",
...
function to_page(path, page) {
    $.ajax({
        url: path + "/maven06/admin/user/showjson",
        data: "page=" + page,
        type: "get",
        success: function (result) {
...
function build_page_nav(path,result) {
    ...
    if(!result.info.pageInfo.hasPreviousPage) {
        ...
        prePage.click(function () {
            to_page('/maven06',result.info.pageInfo.prePage);
        });
    }
    ...
    if(!result.info.pageInfo.hasNextPage) {
        ...
        nextPage.click(function () {
            to_page('/maven06',result.info.pageInfo.nextPage);
        });
    }

    lastPage.click(function () {
        to_page('/maven06',result.info.pageInfo.lastPage);
    });
